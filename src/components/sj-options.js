...
class Options extends Component {
  ...
  startGameStrict() {
    ...
    let activate = () => {
      ...
      if (index == 0) {
        document.getElementById(index).classList.toggle(styles.cpu0)
      }
      if (index == 1) {
        document.getElementById(index).classList.toggle(styles.cpu1)
      }
      if (index == 2) {
        document.getElementById(index).classList.toggle(styles.cpu2)
      }
      if (index == 3) {
        document.getElementById(index).classList.toggle(styles.cpu3)
      }
    }

    let sound = () => {
      if (this.props.cpuSequence[0] == 0) {
        let tone0 = new Audio(require('../assets/simonSounds0.mp3'))
        tone0.crossOrigin = 'anonymous'
        tone0.play()
      }
      if (this.props.cpuSequence[0] == 1) {
        let tone1 = new Audio(require('../assets/simonSounds1.mp3'))
        tone1.play()
      }
      if (this.props.cpuSequence[0] == 2) {
        let tone2 = new Audio(require('../assets/simonSounds2.mp3'))
        tone2.play()
      }
      if (this.props.cpuSequence[0] == 3) {
        let tone3 = new Audio(require('../assets/simonSounds3.mp3'))
        tone3.play()
      }
    }
